...
export default function RecoveryResources() {
    ...
    const categories = state.recoveryResourceCategories || [];
    ...
    const usedCategories = [...new Set(resources.map(r => r.category || 'Uncategorised'))];
    ...
    const handleAddCategory = (e) => {
        ...
        dispatch({ type: ACTIONS.ADD_RECOVERY_RESOURCE_CATEGORY, payload: { name: newCatName.trim(), sortOrder: categories.length } });
        setNewCatName('');
    };
    ...
    const handleUpdateCategory = (item) => {
        ...
        dispatch({ type: ACTIONS.UPDATE_RECOVERY_RESOURCE_CATEGORY, payload: { ...item, name: newName } });
        (state.recoveryResources || []).forEach(r => {
            if (r.category === oldName) {
                dispatch({ type: ACTIONS.UPDATE_RECOVERY_RESOURCE, payload: { id: r.id, category: newName } });
            }
        });
    ...
    const handleDeleteCategoryClick = (cat) => {
        const filesInCat = (state.recoveryResources || []).filter(r => r.category === cat.name);
        if (filesInCat.length > 0) {
            ...
            dispatch({ type: ACTIONS.DELETE_RECOVERY_RESOURCE_CATEGORY, payload: cat.id });
        }
    };
    ...
    const handleConfirmDelete = () => {
        ...
        (state.recoveryResources || []).forEach(r => {
            if (r.category === deleteConfirm.catName) {
                dispatch({ type: ACTIONS.UPDATE_RECOVERY_RESOURCE, payload: { id: r.id, category: targetCat } });
            }
        });
        dispatch({ type: ACTIONS.DELETE_RECOVERY_RESOURCE_CATEGORY, payload: deleteConfirm.catId });
    ...
    const handleUpload = (e) => {
        ...
        uploadFiles.forEach(file => {
            dispatch({
                type: ACTIONS.ADD_RECOVERY_RESOURCE,
                payload: {
                    name: file.name,
    ...
    const catColor = (name) => {
        const colors = ['var(--success)', 'var(--info)', 'var(--warning)', 'var(--primary)', 'var(--text-muted)', 'var(--danger)'];
        ...
        <>
            ...
            <div className="page-body">
                {displayCategories.map(catName => {
                    const items = resources.filter(r => (r.category || 'Uncategorised') === catName);
                                    ...
                                    <div className="resource-file-list">
                                        {uploadFiles.map((f, i) => (
                                            <div key={i} className="resource-file-item">
                                                <FileText style={{ width: 16, height: 16, color: 'var(--success)', flexShrink: 0 }} />
                    ...
                    <form onSubmit={handleSaveEdit}>
                        <div className="modal-body">
                            ...
                            <div className="form-row">
                                <div className="form-group">
                                    <label className="form-label">Category</label>
                                    <select className="form-select" name="category" defaultValue={editItem.category || 'Uncategorised'}>
                                        {categories.map(c => <option key={c.id} value={c.name}>{c.name}</option>)}
                                        <option value="Uncategorised">Uncategorised</option>
                                    </select>
                                </div>
                                    ...
                                    {editingCatId === cat.id ? (
                                        ...
                                        <>
                                            ...
                                            <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                                                {(state.recoveryResources || []).filter(r => r.category === cat.name).length} files
                                            </span>
